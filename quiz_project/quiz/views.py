...
def quiz_detail_view(request, quiz_id):
    ...
    if request.method == 'POST':
        ...
        if existing_result:
            # If an existing result exists, update it with the new score
            existing_result.score = score_percentage
            existing_result.feedback = "Your quiz submission has been processed."
...
def result_view(request, quiz_id):
    ...
    user_result = Result.objects.filter(user=request.user, quiz=quiz).first()  # Retrieve the first result
    score = None
    feedback = None
    if user_result:
        score = user_result.score
        feedback = user_result.feedback
    return render(request, 'quiz/result.html', {'quiz': quiz, 'score': score, 'feedback': feedback})
